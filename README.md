...
В киберспортивных трансляциях (CS2, шоу-матчи, студийные съёмки) часто есть несколько POV-камер,  
снятых с отдельных игроков. Обычно режиссёр или оператор вручную переключает эти камеры в OBS.  
**CS2 → OBS Router** делает это автоматически:

- когда игрок меняется в игре,  
- OBS мгновенно переключает нужную камеру,  
- при этом выключаются все остальные POV.

Это экономит время, снижает риск ошибок и делает продакшен максимально плавным.  
...
Разработано для автоматизации киберспортивных трансляций **Counter-Strike 2**  
Система совместима с **Lexogrine HUD Manager**, **OBS WebSocket v5**,  
и может быть адаптирована под другие игры с GSI API.
